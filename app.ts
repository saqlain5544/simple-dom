...
class Utils {
    ...
    static hasOwn(prop:string, obj:object) {
        return obj.hasOwnProperty(prop);
    }

    static each(array, callback: Function) {
     ...
     */
    static getStyle(element: HTMLElement, property: string) {
        //@ts-ignore
...
class Dom {
    length: number;
    element: HTMLElement | HTMLAllCollection | Document | Window;
    ...
    init(selector:string, context?) {
        ...
        switch (selector) {
            ...
            default:
                //@ts-ignore
                let element = <Array>Sizzle(selector, context);
                this.length = <number>element.length;
     ...
     */
    position():any {
        let element = <HTMLElement>this.getFirst();
        if ( !element ) {
            return null;
        }
...
let dom = function (selector: string, context?) {
